...
def _riccati_rhs(gamma, omega, Delta, hvf):
    """RHS of hbar v_F d(gamma)/ds = Delta - 2 omega gamma - conj(Delta) gamma^2."""
    return (Delta - 2.0 * omega * gamma - np.conj(Delta) * gamma * gamma) / hvf


def integrate_riccati(omega, Delta_s, hvf, ds, gamma0):
    """
    @fn integrate_riccati
    @brief Integrate the Riccati coherence function gamma forward along one
    quasiclassical trajectory with RK4.  This is the core inhomogeneous solver
    (the vortex-lattice driver calls it per trajectory).

    For gamma-tilde, call this routine with conj(Delta) along the *reversed*
    trajectory and reverse the result (gammat behaves like gamma under
    s -> -s, Delta -> conj(Delta)).

    @param   omega: renormalized Matsubara frequency w_tilde (scalar, > 0)
    @param Delta_s: complex gap Delta_tilde sampled along the trajectory [Ns]
    @param     hvf: hbar * |v_F| (> 0) [eV * length]
    @param      ds: arc-length step (> 0) [length]
    @param  gamma0: initial value at s[0] (bulk root upstream of the disturbance)
    @return gamma_s: gamma along the trajectory [Ns]
    """
    Ns = len(Delta_s)
    gamma_s = np.empty(Ns, dtype=np.complex128)
    g = complex(gamma0)
    gamma_s[0] = g
    for i in range(Ns - 1):
        D0 = Delta_s[i]
        D1 = Delta_s[i + 1]
        Dm = 0.5 * (D0 + D1)
        k1 = _riccati_rhs(g, omega, D0, hvf)
        k2 = _riccati_rhs(g + 0.5 * ds * k1, omega, Dm, hvf)
        k3 = _riccati_rhs(g + 0.5 * ds * k2, omega, Dm, hvf)
        k4 = _riccati_rhs(g + ds * k3, omega, D1, hvf)
        g = g + ds / 6.0 * (k1 + 2.0 * k2 + 2.0 * k3 + k4)
        gamma_s[i + 1] = g
    return gamma_s


def riccati_trajectory_gf(omega, Delta_s, hvf, ds):
    """
    @fn riccati_trajectory_gf
    @brief Solve both coherence functions along a trajectory and return the
    quasiclassical propagators (g, f, ft) at every point.

    gamma is integrated forward (stable along +v_F) from the upstream bulk root;
    gammat is integrated forward over the reversed trajectory with conj(Delta)
    and flipped back.  Bulk (homogeneous) roots at the trajectory ends provide the
    inflow boundary conditions, which the Riccati equation forgets exponentially
    over a coherence length -- so a trajectory spanning several xi is self-healing.

    @param   omega: renormalized Matsubara frequency (scalar, > 0)
    @param Delta_s: gap sampled along the trajectory [Ns] complex
    @param     hvf: hbar |v_F| [eV*length]
    @param      ds: arc-length step [length]
    @return (g, f, ft): propagators along the trajectory [Ns]
    """
    g0_a, _ = riccati_homogeneous(omega, Delta_s[0])
    gamma = integrate_riccati(omega, Delta_s, hvf, ds, g0_a)
    # gammat: reversed trajectory, conj(Delta); upstream end is the last point
    _, g0_b = riccati_homogeneous(omega, Delta_s[-1])
    gammat_rev = integrate_riccati(omega, np.conj(Delta_s[::-1]), hvf, ds, g0_b)
    gammat = gammat_rev[::-1]
    return propagators_from_riccati(gamma, gammat)
