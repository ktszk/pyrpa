...
contains
  ...
  subroutine get_rayleigh(rq,delta,newdelta)
    !> Rayleigh quotient: rq = Re(<del, newdel>) assuming ||del||=1
    complex(c_double),intent(in),dimension(Nk,Nw,Norb,Norb):: delta,newdelta
    ...
    do l=1,Norb
       do m=1,Norb
          ...
          do j=1,Nw
             do i=1,Nkall
                tmp=tmp+dble(conjg(delta(invk(1,i),j,m,l))*newdelta(invk(1,i),j,m,l))
             end do
          end do
          !$omp end do
       end do
  ...
  subroutine solve_arnoldi(m_arnoldi)
    ...
    H_mat=(0.0d0,0.0d0)
    ! v_0: normalized initial gap function (already unit-norm from get_initial_delta)
    ...
    do j=0,m_dim-1
       ...
       if(j<m_dim-1)then
          !$omp parallel workshare
          V(:,:,:,:,j+1)=newdelta(:,:,:,:)*(1.0d0/beta)     ! v_{j+1} = w / beta
          !$omp end parallel workshare
       end if
    end do
    ...
    ! build delta1 = Ritz vector for lambda_-, store in host delta1 for deflation in Pass 2
    !$omp parallel workshare
    delta1(:,:,:,:)=(0.0d0,0.0d0)
    !$omp end parallel workshare
    do ii=1,m_act
       !$omp parallel workshare
       delta1(:,:,:,:)=delta1(:,:,:,:)+VR(ii,idx)*V(:,:,:,:,ii-1)   ! delta1 = V_m * y_-
       !$omp end parallel workshare
    end do
    call get_norm(beta,delta1)
    !$omp parallel workshare
    delta1(:,:,:,:)=delta1(:,:,:,:)*(1.0d0/beta)               ! normalize deflation vector
    !$omp end parallel workshare
...
subroutine mkfk_trs_nsoc(fk,Gk,delta,Nk,Nw,Norb) bind(C,name="mkfk_trs_nsoc_")
  ...
  do j=1,Nw
     do i=1,Nk
        ! cmat1 = G(k,iw) * Delta(k,iw)
        call zgemm('N','N',Norb,Norb,Norb,(1.0d0,0.0d0),Gk(i,j,:,:),Norb,delta(i,j,:,:),Norb,(0.0d0,0.0d0),cmat1,Norb)
        ! fk = -cmat1 * G(k,iw)^H
        call zgemm('N','C',Norb,Norb,Norb,(-1.0d0,0.0d0),cmat1,Norb,Gk(i,j,:,:),Norb,(0.0d0,0.0d0),fk(i,j,:,:),Norb)
     ...
     do m=1,Nchi
        if(olist(l,1)>=olist(m,2))then
           ...
           do i=1,Nkall
              ...
              tmpVdelta(kmap(1,i),kmap(2,i),kmap(3,i),Nw+1)=0.5d0*(Smat(m,l)+sgn*Cmat(m,l)) !Nw+1 consider w_n=>inf limit
           end do
           !$omp end do
           !$omp do private(i,j)
           do j=2,Nw
...
subroutine get_initial_delta(delta,init_delta,uni,kmap,invk,Nkall,Nk,Nw,Norb,gap_sym) bind(C,name="get_initial_delta_")
  ...
  if(gap_sym==0)then
     ...
     !$omp parallel do private(i,l,m,n,tmpu)
     do i=1,Nk
        do n=1,Norb
           tmpu(:,n)=uni(:,n,i)*init_delta(i,n)
        end do
        call zgemm('N','C',Norb,Norb,Norb,(1.0d0,0.0d0),tmpu,Norb,uni(:,:,i),Norb,(0.0d0,0.0d0),delta0(i,:,:),Norb)
        do l=1,Norb
