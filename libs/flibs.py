...
def gen_green_from_eig(selfen,eig,uni,mu,temp):
    ...
    flibs.gen_green_inv_from_eig(Gk,selfen,uni,eig,byref(c_double(mu)),
                                 byref(c_double(temp)),byref(c_int64(Nk)),
...
def get_chi0_conv(Gk,kmap,olist,temp,Nx,Ny,Nz):
    ...
    flibs.get_chi0_conv_(chi,Gk,kmap,olist,byref(c_double(temp)),byref(c_int64(Nx)),
                        byref(c_int64(Ny)),byref(c_int64(Nz)),byref(c_int64(Nw)),
...
def linearized_eliashberg(Gk,uni,Smat,Cmat,olist,kmap,invk,Nx,Ny,Nz,temp,gap_sym,eps=1.0e-3,itemax=300):
