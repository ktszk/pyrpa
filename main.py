...
def get_mass(mesh,rvec,ham_r,S_r,mu:float,de=3.e-4,meshkz=20):
    ...
    def calc_mc(kz_ext,band_idx):
        ...
        if Sp is None or Sm is None:
            return None
        dSdE_SI=(Sp-Sm)/(2.*de)*1.e20/eV2J   # m^-2/J
        return hbar_SI**2/(2.*np.pi)*np.abs(dSdE_SI)/scconst.m_e
    ...
    for band_idx in sorted(S_scan.keys()):
        ...
        print(f"Band {band_idx}: extremal kz = {[f'{k:.4f}' for k in cand_kz]}",flush=True)
