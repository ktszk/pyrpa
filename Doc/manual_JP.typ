...
- `orb_dep` （bool）: `True`にすると軌道依存の相互作用行列 `Umat`, `Jmat` を使用します。`False`（デフォルト）では `U`, `J` の一定値を全軌道に適用します。`Umat` の対角は軌道内 $U_(i i)$, 非対角は軌道間 $U'_(i j)$ そのもの（$U' = U - 2J$ の金森関係は再適用されません）, `Jmat` は $J_(i j)$ です。いずれも実数行列で, 大きさは $N_"orb" times N_"orb"$, または `site_prof` が同じ軌道数のサイトを複数並べている場合は1サイト分の大きさでも構いません（その場合は各サイトに同じ行列が複製されます。例: 5軌道のセットを2-Fe・10軌道模型へ）。頂点関数が読むのは同一サイト上の軌道対のみなので, サイト間ブロックは参照されません。

- `UJ_material` （文字列, 任意）: `orb_dep=True` のとき, `Umat`, `Jmat` を直接書く代わりに保存済みのパラメータセット（鉄系超伝導体の cRPA $U$/$J$ など）を読み込みます。読み込み元は `UJ_dir`（デフォルト `UJ`）で, 物質ごとのファイルを置いたディレクトリ `{UJ_dir}/{UJ_material}.json`, または複数物質を物質名のキーで納めた単一の json ファイルのどちらでも構いません。各エントリの `U`, `J` はフラット（行優先で $N_"orb"^2$ 個）でも入れ子の行リストでも記述できます。同梱セット: `FeSe`, `FeTe`, `LiFeAs`, `BaFe2As2`, `LaFeAsO`, `LaFeAsOnakamura`（Fe-$d$ 5軌道）。いずれも1サイト分のセットで, 多サイト模型では `orb_dep` の項で述べたように各サイトへ複製されます。ファイルの軌道順はハミルトニアンの軌道順と一致している必要があります。`None`（デフォルト）: ヘッダに書かれた `Umat`, `Jmat` を使用します。
...
- `eil_gap_file` （`None` / 文字列）: option=15/23（`LIN_ELIASHBERG`/`NONLIN_ELIASHBERG`）を `sw_out_self=True` で実行したとき `output_gap_wannier` が Wannier 実空間「ホッピング」形式で書き出した自己無撞着 RPA/FLEX ギャップの基底名（拡張子なし, 例 `'gap_wannier'`）。指定すると $Delta(bold(R), i omega_n)$ を読み込み, その逆フーリエ変換 $Delta_"orb"(bold(k)) = sum_bold(R) e^(-i 2 pi bold(k) dot bold(R)) Delta(bold(R))$ を FS バンドへ射影して `eil_gap_orbital` として用いる（負符号は書き出し側の `np.fft.ifftn` の厳密な逆変換で, 書き出し→読み戻しの往復で検証済み）。*以前計算した RPA ギャップ*（例: $"KFe"_2"As"_2$, PRB *84*, 144514）を渦のペアリング形状因子に使うための経路。ギャップを書き出した RPA 計算と Eilenberger 計算は*同一*の Wannier Hamiltonian（同じ軌道基底・$bold(R)$/$bold(a)$ 規約, できれば $mu$/filling）を使う必要がある（バンド固有ベクトルと $Delta_"orb"$ が同じ基底であるため）。`eil_gap_orbital`/`gap_sym` に優先。ファイル中の軌道ペアは物理的な順序 $Delta_(a b)$ で格納される（書き出し時に Fortran 配列の反転ビューを一度だけ転置し, npz に `orb_conv` を刻印する）。刻印が無い旧形式のギャップも正しく読めるが, その旨が1行表示されるので気になる場合は書き出し直すこと。この順序は無害な付け替えではない: $Delta$ はエルミートだが対称ではなく, 転置で反対称成分の符号が反転する（000AsP の RPA 解では $max |Delta|$ の 63%。完全ギャップの $s_(plus.minus)$ ポケットがノードを持つように見えてしまう）。
