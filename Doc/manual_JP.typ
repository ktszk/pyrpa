...
option=24〜26 は超伝導の*準古典 Eilenberger 方程式*（エネルギー積分された Gor'kov 方程式）を解きます。ギャップや不純物がフェルミ波長スケールでゆっくり変化する（$Delta, T_c, hbar\/tau << E_F$）状況で有効で, $T_c$・状態密度・表面 Andreev 束縛状態・渦／渦格子状態を扱う自然な枠組みです。未知数は準古典伝播関数 $g(bold(k)_F, bold(r), omega_n)$（normal）と $f$（anomalous）で, 単一の*Riccati 振幅* $a$（$f = 2a\/(1+a a^*)$, $g = (1-a a^*)\/(1+a a^*)$）で表し, フェルミ速度に沿った直線軌道上を数値的に安定な Fortran カーネル（`riccati_chords`; d-vector 用には $2 times 2$ スピン版 `matrix_riccati_chords`）で積分します。ペアリングは可分離形 $Delta(bold(k)_F, bold(r)) = Delta(bold(r)) phi(bold(k)_F)$ で, 形状因子 $phi$ は `gap_sym` で決まり $angle.l |phi|^2 angle.r_"FS" = 1$ に規格化されるため, 結合定数 `eil_coupling` は無次元の $lambda$ です。
...
- `eil_vort_lattice_sc=True` ＋ `eil_field_list`: *真の周期*磁気 Bloch 渦格子（定式化 A, 極端第 II 種）: 各コアに実ノードを持つ複素秩序変数 $Psi(bold(r))$ と完全な Abrikosov 超流動位相, $B\/H_(c 2)$ を走査して $angle.l N(0) angle.r (B)$ を得る（$d$ 波 $tilde sqrt(B)$ Volovik）。`eil_lattice` は `'square'`/`'triangular'`, `eil_nvortex` はセルあたり磁束量子数, 有限 `eil_kappa` で London 遮蔽を追加（`eil_vort_scA=True` で $bold(A)$ を準古典電流 $bold(j)_s = angle.l bold(v)_F "Im" g angle.r$ から完全自己無撞着に, `je` の `A_renew` 法）。
