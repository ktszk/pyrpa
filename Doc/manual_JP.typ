...
- `sw_rescale_flex` のリスケールは発散する特定モードだけでなく全運動量・全周波数の $chi_0$ を一様に縮めます。リスケールが発動した計算（標準出力に `[FLEX] Stoner factor= ... : rescaling chi0` と表示されます）では自己エネルギーの絶対値も一様に弱められるため, 磁気不安定点近傍の結果は定性的なものとして解釈してください。
...
- 松原周波数方向の畳み込み（$chi_0$, $Sigma$, Eliashberg カーネル）は長さ $2 N_w$ の FFT による循環畳み込みで, 高周波 tail 補正は行いません（$V$ の最遠点のみ bare 頂点で近似）。実効カットオフは $omega_c = (2 N_w - 1) pi T$ と温度に比例して縮むため, $N_w$ 固定の温度走査には $O(1\/N_w)$ の系統誤差が乗ります。最低温度でも $omega_c$ がバンド幅と $U$ を十分上回るように $N_w$ を選んでください。
...
- `sw_rescale_flex` （bool）: option=14 の FLEX 計算で, 反復中に Stoner 因子が 1 に達した場合に $chi_0$ 全体を一律に $(1-10^(-4))\/S$ 倍して発散を防ぎます。Stoner 因子が 1 に近い系で有効ですが, 全 $(bold(q), i nu_n)$ の $chi_0$ を一様に変更する正則化なので, リスケールが発動した結果は磁気不安定点近傍の定性的なものとして扱ってください（option=14 の注意書き参照）。
