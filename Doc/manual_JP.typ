...
を構築した上で RPA で動的スピン感受率 $chi_s^"SC" = chi_0^"SC" \/ (1 - S chi_0^"SC")$ を求め, symmetry line 沿いにスペクトルを描画します。$Delta$ の最大振幅は `delta0` で指定し（meV スケール推奨）, `gap_sym` で初期対称性を選びます（負値で triplet）。出力は `chis_sc_spec.png`。

=== option=13: 超伝導状態の特定 $bold(q)$ 点スピン感受率 (`CHIS_QPOINT_SC`)

option=12 と同じ SC 状態の枠組みで, `at_point` で指定した $bold(q)$ 点での $chi_s^"SC"(bold(q), omega)$ の周波数依存性を計算・プロットします。
...
- 各反復: $Delta_"new" = T \/ N_k sum V_Delta dot F$ → DIIS 混合 → $Sigma$ 更新 → SC Dyson で $G, F$ 更新 → $chi^"GG", chi^"FF"$ から $V_sigma, V_Delta$ 再構築
- `m_diis_num`: DIIS 履歴長（$>= 2$ で Pulay 外挿, それ以下は線形混合 pp=0.3）
- `sw_self=True`: FLEX 自己エネルギー込みで Σ-dressed Green's function を使う
- `sw_from_file=True`: `sigma.bin` から自己エネルギーを読み込む
- 初期 Δ には線形 Eliashberg の収束ギャップ関数を使うのが推奨（caller 側で `gap.npy` から構築）
...
- `delta0` （実数, 単位: eV）: option=12,13 の SC chi 計算で用いる初期ギャップ関数の最大振幅。物理的には超伝導ギャップサイズに対応します（典型値: $10^{-3} - 10^{-2}$ eV ≒ 1–10 meV）。0 にすると常状態と等価になります。
...
線形 Eliashberg で $lambda approx 1$ となる温度より低温に降ろし, $Delta$ を有限振幅まで成長させる場合は option=23 を使用します。

```python
# ステップ 1: 線形 Eliashberg で初期ギャップを取得
option = 15
gap_sym = 1
sw_out_self = True

# ステップ 2: 非線形 Eliashberg ループ
option = 23
sw_self = True            # FLEX 自己エネルギー込み
sw_from_file = True
m_diis_num = 5            # DIIS Pulay 加速
```

非線形ループは前ステップで保存した `gap.npy` を初期 $Delta$ として読み込みます。低温（$T tilde.equiv T_c \/ 5$）では DIIS 履歴を 5–10 程度にすると収束が速くなります。
...
- *非線形 Eliashberg が発散する*: 初期 $Delta$ を線形 Eliashberg の収束解から取る, `m_diis_num` を 5–10 に増やす, `tempK` を上げる, `Nw` を増やすなどを試してください。Stoner 因子（`SDW` の出力値）が 1 を超えている場合は磁気不安定なので `U` を下げる必要があります。
