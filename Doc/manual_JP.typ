...
- 温度アニーリング: 同一 `Nw` での小刻みな冷却（$T_"new"\/T_"old" gt.eq 0.8$ 程度）なら `sigma.bin` の生読み込みで十分です。生読み込みは周波数軸を $T_"old"\/T_"new"$ 倍に圧縮した seed に相当し, この圧縮が冷却に伴う $Sigma$ の増大を近似的に模倣するため, 近臨界では忠実な補間よりむしろ収束が速いことを確認しています。`Nw` を変える場合（生読み込みでは読めない唯一のケース）は Python ヘルパー `plibs.regrid_sigma_bin(temp_old, temp_new, Norb, Nw_old, Nw_new, w_scale)` で `sigma.bin` を新グリッドへ再補間できます（共役対称性 $Sigma_(l m)(-i omega) = Sigma_(m l)(i omega)^*$ による負周波数拡張つき 3 次スプライン; 循環畳み込みの折返しアーチファクトを持つ最終周波数点は除去し, カットオフ超過分は内側窓でフィットした $c_0 + c_1\/(i omega)$ tail で補完）。近臨界の冷却では `w_scale=temp_old/temp_new`（生読み込みと同じ圧縮）を推奨, 忠実な補間は `w_scale=1`（既定）です
